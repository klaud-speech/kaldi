...
for x in train dev test; do 
  ...
  copy-feats scp:$mfcc_txt_dir/$x"_"local.scp ark,t:- | add-deltas ark,t:- ark,t:- > $x.mfcc39
done

# Now make 40-dim and 120-dim log Mel filter bank features.
fbankdir=fbank
fbank_txt_dir=fbank_txt
...
for set in train dev test; do
  ...
  copy-feats scp:$fbank_txt_dir/$set"_"local.scp ark,t:- > $set.fbank40
  if $add_deltas; then
    copy-feats scp:$fbank_txt_dir/$set"_"local.scp ark,t:- | add-deltas ark,t:- ark,t:- > $set.fbank120
  fi
done
